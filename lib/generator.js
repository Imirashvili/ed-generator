...
function buildNewsDateTime(blockDays) {
  ...
  // условие группировки: подряд + 1 слот + 1 слот + одинаковое время
  if (
    consecutive &&
    d1.slots.length === 1 &&
    d2.slots.length === 1 &&
    d1.slots[0].key === d2.slots[0].key
  ) {
    const monthWord = MONTHS_GEN[d1.date.m - 1];
    if (sameMonth) {
      // "20 и 21 января с 19:00 до 21:00"
      return `${d1.date.d} и ${d2.date.d} ${monthWord} ${d1.slots[0].timeText}`;
    }
    // на случай разных месяцев (маловероятно для подряд), но корректно:
    return `${formatDateHuman(d1.date)} и ${formatDateHuman(d2.date)} ${d1.slots[0].timeText}`;
  }

  // иначе — как раньше: "20 января ... и 21 января ..."
