...
function Button({ children, onClick, variant = "secondary", className = "", disabled, title, type }) {
  const base =
...
export default function AppHome() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // если не админ — запретить вкладку встреч
      if (!prof.is_admin && tab === "vstrecha") {
        setTab("obhod");
      }
    })();
  ...
  const allowedTabs = useMemo(() => {
    return TABS.filter((t) => (t.key === "vstrecha" ? isAdmin : true));
  }, [isAdmin]);
    ...
    if (tab === "obhod") {
      ...
      if (manual.has(scenarioKey)) {
        ...
        if (scenarioKey === "reschedule") {
          const groups = new Map(); // `${address}||${date_raw}` -> { address, date_raw, times[], rowNums[] }
                    ...
                    <table className="w-full text-sm border-collapse">
                      <thead className="bg-gray-50">
                        <tr className="text-gray-600">
                          ...
                          <th className="text-left p-2 border-b border-gray-200 w-[160px]">Дата</th>
                        </tr>
                      </thead>
                    ...
                    <div className="rounded-md border border-gray-200 bg-white p-3">
                      ...
                      <div className="grid gap-2">
                        ...
                        <Button onClick={() => copyWithFeedback(`push-title-${selectedIdx}`, selected.push_title)}>
                          {copiedKey === `push-title-${selectedIdx}` ? "Заголовок скопирован" : "Скопировать заголовок пуша"}
                        </Button>

                        <Button onClick={() => copyWithFeedback(`push-body-${selectedIdx}`, selected.push_body)}>
                          {copiedKey === `push-body-${selectedIdx}` ? "Текст скопирован" : "Скопировать текст пуша"}
                        </Button>
                      </div>
                    </div>

                    <div className="text-xs text-gray-500">
                      Совет: для админки — копируй заголовок + HTML в редактор новости,
